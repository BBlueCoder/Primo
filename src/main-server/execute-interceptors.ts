...
import { Interceptor, InterceptorRequest, InterceptorResponse } from "./interceptors";
...
export function executeInterceptorsRecursive(
    ...
    path: string,
    index: number,
    interceptors: Map<string, Interceptor<InterceptorRequest, InterceptorResponse>[]>,
    callback: () => void,
) {
    ...
    if (intps && index < intps!.length) {
        intps![index].intercept(req, res, () => {
            executeInterceptorsRecursive(req, res, path, index + 1, interceptors, callback);
        })
    } else {
        callback();
    }
