...
function getDestinationForSavingFiles(
    req: CustomRequest,
    fieldName: string,
    filename: string,
    mimeType: string
    ...
    if (
        req.requestOpts !== undefined &&
        req.requestOpts.destination !== undefined
    ) {
...
function getFilenameForSavingFiles(
    req: CustomRequest,
    filename: string,
    mimeType: string
) {
    if (
        req.requestOpts !== undefined &&
        req.requestOpts.filename !== undefined
    ) {
...
export function parseFormData(req: CustomRequest, callback: () => void) {
    ...
    bb.on('file', (fieldName, fileStream, info) => {
        ...
        const filename = getFilenameForSavingFiles(
            req,
            info.filename,
            info.mimeType
        )
        ...
        pipeline(fileStream, ws, (err) => {
            if (err) console.log(err)
                ...
                fs.stat(savePath, (err, stats) => {
                    ...
                    files.pop()
                    if (isClosed && files.length === 0) callback()
                })
            }
        })
    })
