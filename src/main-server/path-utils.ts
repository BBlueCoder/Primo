export class PathUtils {
    ...
    public static extractParamsFromUrlPath(
        pathRegex: string,
        url: string
    ): string[] {
        const extractedResult = new RegExp(pathRegex).exec(
            PathUtils.wrapPath(url)
        )
        const params: string[] = []
    ...
    public static extractQueryParamsFromUrl(query: object): {
        [name: string]: string
    } {
    ...
    public static isPathMatchToPattern(pattern: string, path: string): boolean {
        ...
        if (patternArrays[lastIndex] == '*') {
            for (let i = 0; i < pathArrays.length; i++) {
                if (i >= patternArrays.length) return false
                if (
                    patternArrays[i] != '*' &&
                    patternArrays[i] != pathArrays[i]
                )
                    return false
            }
        } else if (patternArrays[lastIndex] == '**') {
            for (let i = 0; i < pathArrays.length; i++) {
                if (patternArrays[i] == '**') break

                if (patternArrays[i] != pathArrays[i]) return false
            }
        } else {
            if (pathArrays.length != patternArrays.length) return false
    ...
    public static getPatternThatMatchesPath(
        patterns: IterableIterator<string>,
        path: string
    ): string {
        for (const p of patterns) {
            if (this.isPathMatchToPattern(p, path)) return p
        }
