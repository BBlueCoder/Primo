...
describe('path utils', () => {
    ...
    describe('extractParamsFromUrlPath()', () => {
        it('should return params from a given url according to the path regex', () => {
            const params = PathUtils.extractParamsFromUrlPath(
                expectedRegex + '',
                '/pathname/25/100'
            )
    ...
    describe('isPathMatchToPattern', () => {
        it('should return true when path matches with pattern', () => {
            assert.strictEqual(
                PathUtils.isPathMatchToPattern('/pathname', '/pathname/'),
                true
            )
            assert.strictEqual(
                PathUtils.isPathMatchToPattern('/pathname/', '/pathname'),
            ...
            assert.strictEqual(
                PathUtils.isPathMatchToPattern('/pathname/*', '/pathname/5'),
                true
            )
            assert.strictEqual(
                PathUtils.isPathMatchToPattern(
                    '/pathname/*',
                    '/pathname/5?page=1&size=20'
                ),
                true
            )
            assert.strictEqual(
        ...
        it('should return false when path do not match with pattern', () => {
            assert.strictEqual(
                PathUtils.isPathMatchToPattern('/pathname/*', '/invalid'),
                false
            )
            assert.strictEqual(
                PathUtils.isPathMatchToPattern('/pathname', '/pathname/5'),
            ...
            assert.strictEqual(
                PathUtils.isPathMatchToPattern('/pathname/*', '/pathname/5/10'),
                false
            )
            assert.strictEqual(
                PathUtils.isPathMatchToPattern('/pathname/**', '/invalid/5/10'),
                false
            )
        })
    })
